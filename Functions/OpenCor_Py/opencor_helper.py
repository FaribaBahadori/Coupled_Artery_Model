        ...
        for JJ, variables_list in enumerate(variables_list_of_lists):
            ...
            for variable_name in variables_list:
                if variable_name == 'time':
                    ...
                    val = self.simulation.results().states()[variable_name]
                    if val is None or val.values() is None:
                        print(f"Error: state variable '{variable_name}' has no values")
                        sys.exit()
                    results[JJ].append(val.values()[-self.n_steps - 1:].copy())

                elif variable_name in self.simulation.results().algebraic():
                    val = self.simulation.results().algebraic()[variable_name]
                    if val is None or val.values() is None:
                        print(f"Error: algebraic variable '{variable_name}' has no values")
                        sys.exit()
                    results[JJ].append(val.values()[-self.n_steps - 1:].copy())
                    ...
                    print("States:", list(self.simulation.results().states().keys()))
